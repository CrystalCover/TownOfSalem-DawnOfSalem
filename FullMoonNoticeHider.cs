...
namespace Eca.DawnOfSalem
{
    ...
    internal class FullMoonNoticeHider
    {
        ...
        [HarmonyPatch("HandleOnFullMoonNight")]
        [HarmonyPrefix]
        private static bool HandleOnFullMoonNight() => new List<CustomGameSetup.Role>(new[] { WEREWOLF, RANDOM_NEUTRAL, NEUTRAL_KILLING, ANY, JUGGERNAUT, COVEN_RANDOM_NEUTRAL, COVEN_NEUTRAL_KILLING, COVEN_ANY }).FindAll(role => RoleList.Contains(Roles[(int)role])).Any();
    }
}